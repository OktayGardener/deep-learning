...
object Main {
  def main(args: Array[String]) {
    ...
    val fSlicer = new VectorSlicer().setInputCol("allFeatures").setOutputCol("ffeatures").setIndices(Array(1,2,3))

    val myLR = new LinearRegression().setMaxIter(10).setRegParam(0.1).setElasticNetParam(0.1)
    val poly = new PolynomialExpansion().setInputCol("ffeatures").setOutputCol("features").setDegree(2)
    
    
    val lrStage = Array(regexTokenizer, arr2Vect, lSlicer, v2d, lShifter, fSlicer, poly, myLR)
    val pipeline = new Pipeline().setStages(lrStage)
    
    val paramGrid = new ParamGridBuilder().addGrid(myLR.maxIter, Array(1, 4, 7, 10, 13, 19, 100, 1000)).addGrid(myLR.regParam, Array(0.0, 0.1, 0.4, 0.9, 1.5, 10, 100)).build()
